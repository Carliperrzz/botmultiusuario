...
// ✅ Normaliza o usuário para NUNCA quebrar o painel
// Aceita tanto:
//  - { user: { name, role, username } }
//  - { username, role } (como você está passando do server.js)
function normalizeUser({ user, username, role } = {}) {
  const u = user || {};
  const safeUsername = username || u.username || 'Usuário';
  const safeRole = role || u.role || 'viewer';
  const safeName = u.name || safeUsername;
  return { username: safeUsername, role: safeRole, name: safeName };
}

function layoutMobile({ title = 'Painel', user, username, role, bodyHtml = '', footerHtml = '' } = {}) {
  const u = normalizeUser({ user, username, role });
...
<body>
  <div class="top">
    ...
    <div class="tabs">
      ...
      ${u.role === 'admin' ? `<a class="tab" href="/m/users">Usuários</a> <a class="tab" href="/admin">Desktop</a>` : ``}
...
function layoutDesktop({ title = 'Painel', bodyHtml = '' } = {}) {
  return `<!doctype html>
<html lang="pt-br">
<head>
<meta charset="utf-8"/>
